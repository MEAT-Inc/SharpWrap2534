...
namespace SharpWrap2534.J2534Api
{
    ...
    internal class J2534ApiMarshaller
    {
        ...
        // -------------------------------------------------- VERSION 0404/0500 IOCTL MARSHALS ----------------------------------------------------
        ...
        public void PassThruIoctl(uint ChannelId, IoctlId IoctlId, ref PassThruStructs.SConfigList ConfigListStruct)
        {
            ...
            try
            {
                ...
                this.PassThruIoctl(ChannelId, IoctlId, ptrSConfigListNative, IntPtr.Zero);
        ...
        public void PassThruIoctl(uint ChannelId, IoctlId IoctlId, out uint ValueRead)
        {
            ...
            this.PassThruIoctl(ChannelId, IoctlId, IntPtr.Zero, OutputPointer);
        ...
        /// <param name="RescStruct"></param>
        /// <param name="OutputValue"></param>
        public void PassThruIoctl(uint ChannelId, IoctlId IoctlId, PassThruStructs.ResourceStruct RescStruct, out uint OutputValue)
        {
            ...
            ResScturtNative.Connector = (uint)RescStruct.ConnectorType;
            ResScturtNative.NumOfResources = RescStruct.ResourceCount;

            // Assign unallocated memory for resource list
            ResScturtNative.ResourceListPointer = Marshal.AllocHGlobal(sizeof(uint) * RescStruct.ResourceList.Count);
            int[] intArray = RescStruct.ResourceList.ToArray();
            Marshal.Copy(intArray, 0, ResScturtNative.ResourceListPointer, RescStruct.ResourceList.Count);
            ...
            this.PassThruIoctl(ChannelId, IoctlId, RescStructPointer, OutputPointer);
        ...
        public void PassThruIoctl(uint ChannelId, IoctlId IoctlId, PassThruStructs.SByteArray SByteInput, ref PassThruStructs.SByteArray SByteOutput)
        {
            ...
            // use try catch block so we can release the unmanaged memory whatever happens
            try { this.PassThruIoctl(ChannelId, IoctlId, ptrInSByteNative, ptrOutSByteNative); }
            catch (Exception) { throw; }
            finally
            {
        ...
        public void PassThruIoctl(uint ChannelId, IoctlId IoctlId, PassThruStructs.PassThruMsg? InputMessage, ref PassThruStructs.PassThruMsg? OutputMessage)
        {
            ...
            IntPtr ptrIn = IntPtr.Zero; IntPtr ptrOut = IntPtr.Zero;
            PassThruStructsNative.PASSTHRU_MSG inMsgNative; PassThruStructsNative.PASSTHRU_MSG outMsgNative;

            // Check if input message is not null
            if (InputMessage != null)
            {
                inMsgNative = new PassThruStructsNative.PASSTHRU_MSG(-1);
                CopyPassThruMsgToNative(ref inMsgNative, InputMessage);
            ...
            {
                outMsgNative = new PassThruStructsNative.PASSTHRU_MSG(-1);
                CopyPassThruMsgToNative(ref outMsgNative, OutputMessage);
            ...
            finally
            {
                if (OutputMessage != null)
                {
                    ...
                    outMsgNative = (PassThruStructsNative.PASSTHRU_MSG)Marshal.PtrToStructure(ptrOut, typeof(PassThruStructsNative.PASSTHRU_MSG));

                    // copy data back to SByteArray classes
                    CopyPassThruMsgFromNative(ref OutputMessage, outMsgNative);
        ...
        {
            if (SByteInput.numOfBytes > SByteInput.data.Length)
            {
            ...
            inSByteNative.NumOfBytes = SByteInput.numOfBytes;
        ...
        /// </summary>
        public void PassThruIoctl(uint ChannelId, IoctlId IoctlId, ref PassThruStructs.SByteArray outSByte)
        {
            ...
            PassThruStructsNative.SBYTE_ARRAY outSByteNative = new PassThruStructsNative.SBYTE_ARRAY();
            ...
            {
                // copy unmanaged memory to managed struct (SBYTE_ARRAYs)
                outSByteNative = (PassThruStructsNative.SBYTE_ARRAY)Marshal.PtrToStructure(ptrOutSByteNative, typeof(PassThruStructsNative.SBYTE_ARRAY));
