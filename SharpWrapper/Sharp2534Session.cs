...
namespace SharpWrapper
{
    ...
    public class Sharp2534Session   
    {
        ...
        // Private configuration fields for session instances and logging setup
        private readonly SharpLogger _sessionLogger;            // Logger instance for this session
        private static Sharp2534Session[] _sharpSessions;       // Static collection of all the built sessions

        // Public backing fields for session configuration
        public readonly Guid SessionGuid;                       // The GUID of the current session instance
        private readonly string SessionName;                    // The name of the current session instance

        // Private backing fields for our session instance
        private J2534Channel _defaultChannel;                   // The default J2534 channel to open for our session instance

        #endregion //Fields

        #region Properties

        // Public facing properties holding information about our loaded J2534 DLL
        public J2534Dll JDeviceDll { get; private set; }       
        ...
        // Public facing properties holding information about our connected J2534 device
        public J2534Device JDeviceInstance { get; private set; }     
        ...
        // Public facing properties about our session channels, filters, and periodic messages
        ...
        // Status value for the session as a whole. Checks our device and DLL State and returns based on them
        ...
        {
            // Build output string for our device and DLL information and exit out
            return
                $"SharpSession - {this.SessionName}\n" +
        ...
        private Sharp2534Session(JVersion Version, string DllNameFilter, string DeviceNameFilter = "")
        {
            ...
            this.SessionName = $"SharpSession_{this.SessionGuid.ToString("D".ToUpper())}";
            this._sessionLogger = new SharpLogger(LoggerActions.UniversalLogger, this.SessionName);
            this._sessionLogger.WriteLog(this._buildSplittingLine(), LogType.TraceLog);
        ...
        public static void CloseSession(Sharp2534Session SessionToClose)
        {
            ...
            SessionToClose._sessionLogger.WriteLog(SessionToClose._buildSplittingLine(), LogType.TraceLog);
        ...
        public bool PTOpen()
        {
            ...
            this._writeCommandLog("OPENED NEW J2534 INSTANCE OK!", LogType.InfoLog);
            this._writeCommandLog($"DEVICE NAME AND ID: {this.DeviceName} - {this.DeviceId}", LogType.InfoLog);
            this._writeCommandLog($"DEVICE OPEN: {this.JDeviceInstance.IsOpen}");
        ...
        public bool PTClose()
        {
            ...
            this._writeCommandLog("CLOSED OUR J2534 INSTANCE OK!", LogType.InfoLog);
            this._writeCommandLog($"DEVICE OPEN: {this.JDeviceInstance.IsOpen}");
        ...
        public J2534Channel PTConnect(int ChannelIndex, ProtocolId Protocol, PassThroughConnect Flags, BaudRate ChannelBaud, out uint ChannelId)
        {
            ...
            this._writeCommandLog("SENDING OUT PASSTHRU CONNECT METHOD NOW.", LogType.InfoLog);
            this._writeCommandLog($"CONNECT PARAMETERS: {ChannelIndex}, {Protocol}, {ChannelBaud}", LogType.TraceLog);
            ...
            this._writeCommandLog($"PULLED OUT CHANNEL ID: {ChannelId}", LogType.InfoLog);
            this._writeCommandLog("STORING NEWEST CHANNEL AS OUR FALLBACK CHANNEL NOW...", LogType.InfoLog);
        ...
        public void PTDisconnect(int ChannelIndex)
        {
            ...
            this._writeCommandLog($"DISCONNECTING CHANNEL INDEX: {ChannelIndex}", LogType.WarnLog);
        ...
        public J2534Channel PTLogicalConnect(ProtocolId Protocol, PassThroughConnect Flags, PassThruStructs.ISO15765ChannelDescriptor ChannelDescriptor, out uint LogicalChannelId)
        {
            ...
            if (this.DeviceChannels.All(ChannelObj => ChannelObj == null)) {
                this._writeCommandLog("CAN NOT ISSUE LOGICAL COMMANDS ON A DEVICE WITH NO OPENED CHANNELS!", LogType.ErrorLog);
            ...
            if (ParentPhysicalChannel == null) {
                this._writeCommandLog("NO PHYSICAL CHANNEL OBJECTS WERE FOUND TO ISSUE LOGICAL COMMANDS ONTO!", LogType.ErrorLog);
                LogicalChannelId = 0; return null;
            }
            ...
            this._writeCommandLog($"ISSUED A PTLOGICAL CONNECT COMMAND FOR PHYSICAL CHANNEL {ParentPhysicalChannel.ChannelId} WITHOUT ISSUES!", LogType.InfoLog);
            if (BuiltLogicalChannel == null) {
                this._writeCommandLog("FAILED TO BUILD A NEW LOGICAL CHANNEL! THE OUTPUT OBJECT WAS NULL!", LogType.ErrorLog);
                LogicalChannelId = 0; return null;
            }
        ...
        public void PTLogicalDisconnect(uint ChannelId)
        {
            ...
            this._writeCommandLog($"ISSUED A PTLOGICAL DISCONNECT COMMAND FOR PHYSICAL CHANNEL {ParentPhysicalChannel.ChannelId} WITHOUT ISSUES!", LogType.InfoLog);
        }
        ...
        public void PTClearRxBuffer(int ChannelId = -1)
        {
            ...
            if (this.DeviceChannels.All(ChannelObj => ChannelObj == null)) {
                this._writeCommandLog("CAN NOT ISSUE IOCTL COMMANDS ON A DEVICE WITH NO OPENED CHANNELS!", LogType.ErrorLog);
                return;
            }

            // Log Clearing RX buffer, clear it and return 
            J2534Channel ChannelInUse = this._defaultChannel;
            this._writeCommandLog($"CLEARING RX BUFFER FROM DEVICE {this.DeviceName} NOW...", LogType.InfoLog);
            if (ChannelId != -1)
            {
                ...
                if (ChannelInUse != null)
                {
                    ...
                    this._writeCommandLog("CAN NOT CLEAR RX BUFFER FROM NULL CHANNELS!", LogType.ErrorLog); 
                    return;
                }
            }

            // Clear out the channel RX Buffer by the ID here
            this._writeCommandLog($"USING DEVICE INSTANCE {this.DeviceName} FOR BUFFER OPERATIONS", LogType.InfoLog);
            this._writeCommandLog($"CLEARING RX BUFFER FROM CHANNEL ID: {ChannelInUse.ChannelId}!", LogType.WarnLog);
        ...
        {            
            ...
            if (ChannelId != -1)
            {
                ...
                {
                    // Log can't operate on a null channel and exit method
                    this._writeCommandLog("CAN NOT CLEAR TX BUFFER FROM NULL CHANNELS!", LogType.ErrorLog);
                    return;
                }
            }
            ...
            // Clear out the channel RX Buffer by the ID here
            this._writeCommandLog($"USING DEVICE INSTANCE {this.DeviceName} FOR BUFFER OPERATIONS", LogType.InfoLog);
            this._writeCommandLog($"CLEARING TX BUFFER FROM CHANNEL ID: {ChannelInUse.ChannelId}!", LogType.WarnLog);
        ...
        public void PTReadVoltage(out double VoltageRead, bool SilentRead = false, int ChannelId = -1)
        {
            ...
            if (!SilentRead) this._writeCommandLog($"READING VOLTAGE FROM DEVICE {this.DeviceName} NOW...", LogType.InfoLog);

            // Pull voltage value and check for -1
            var VoltageInt = JDeviceInstance.PTReadVBattery();
            if (VoltageInt == -1) {
                this._writeCommandLog("VOLTAGE VALUE WAS -1 THIS IS DUE TO A FAILED IOCTL!", LogType.ErrorLog);
                VoltageRead = -1.0;
                return;
            }
            ...
            if (!SilentRead) this._writeCommandLog($"PULLED VOLTAGE VALUE OF {VoltageRead:F2} OK!", LogType.InfoLog);
        }
        ...
        public void PTSetPins(int PinsToSet, int ChannelId = -1)
        {
            ...
            // Log Clearing RX buffer, clear it and return 
            J2534Channel ChannelInUse = this._defaultChannel;
            this._writeCommandLog($"SETTING PINS ON CHANNEL FOR DEVICE {this.DeviceName} NOW...", LogType.InfoLog);
            if (ChannelId != -1)
            {
                // Check our Device Channel ID
                ChannelInUse = DeviceChannels?.FirstOrDefault(ChannelObj => ChannelObj != null);
                ...
                {
                    // Log can't operate on a null channel and exit method
                    this._writeCommandLog("CAN NOT SET PINS ON NULL CHANNELS!", LogType.ErrorLog);
                    return;
                }
            }
            ...
            this._writeCommandLog($"USING DEVICE INSTANCE {this.DeviceName} FOR BUFFER OPERATIONS", LogType.InfoLog);
            this._writeCommandLog($"SETTING PINS TO {PinsToSet} FOR CHANNEL ID: {ChannelInUse.ChannelId}!", LogType.WarnLog);
        ...
        public bool PTGetConfig(ConfigParamId ConfigParam, out uint ConfigValue, int ChannelId = -1)
        {
            ...
            // Log Clearing RX buffer, clear it and return 
            J2534Channel ChannelInUse = this._defaultChannel;
            this._writeCommandLog($"GETTING CONFIGURATION FROM CHANNEL CHANNEL FOR DEVICE {this.DeviceName} NOW...", LogType.InfoLog);
            if (ChannelId != -1)
            {
                // Check our Device Channel ID
                ChannelInUse = DeviceChannels?.FirstOrDefault(ChannelObj => ChannelObj != null);
                ...
                {
                    // Log can't operate on a null channel and exit method
                    this._writeCommandLog("CAN NOT GET CONFIGURATIONS ON NULL CHANNELS!", LogType.ErrorLog);
                    ConfigValue = 0; return false;
                }
            }
            ...
            // Issue the Set Pins command here
            this._writeCommandLog($"USING DEVICE INSTANCE {this.DeviceName} FOR BUFFER OPERATIONS", LogType.InfoLog);
            this._writeCommandLog($"GETTING CONFIGURATION {ConfigParam} FOR CHANNEL ID: {ChannelInUse.ChannelId}!", LogType.WarnLog); 
        ...
        { 
            ...
            // Log Clearing RX buffer, clear it and return 
            J2534Channel ChannelInUse = this._defaultChannel;
            this._writeCommandLog($"SETTING CONFIGURATION FROM CHANNEL CHANNEL FOR DEVICE {this.DeviceName} NOW...", LogType.InfoLog);
            if (ChannelId != -1)
            {
                // Check our Device Channel ID
                ChannelInUse = DeviceChannels?.FirstOrDefault(ChannelObj => ChannelObj != null);
                ...
                {
                    // Log can't operate on a null channel and exit method
                    this._writeCommandLog("CAN NOT SET CONFIGURATIONS ON NULL CHANNELS!", LogType.ErrorLog);
                    return false;
                }
            }
            ...
            this._writeCommandLog($"USING DEVICE INSTANCE {this.DeviceName} FOR BUFFER OPERATIONS", LogType.InfoLog);
            this._writeCommandLog($"SETTINGS CONFIGURATION {ConfigParam} WITH VALUE {ConfigValue} FOR CHANNEL ID: {ChannelInUse.ChannelId}!", LogType.WarnLog);
        ...
        public bool PTFiveBaudInit(byte InputByte, out byte[] ResponseBytes, int ChannelId = -1)
        {
            ...
            // Log Clearing RX buffer, clear it and return 
            J2534Channel ChannelInUse = this._defaultChannel;
            this._writeCommandLog($"ISSUING FIVE BAUD INIT FOR DEVICE {this.DeviceName} NOW...", LogType.InfoLog);
            if (ChannelId != -1)
            {
                // Check our Device Channel ID
                ChannelInUse = DeviceChannels?.FirstOrDefault(ChannelObj => ChannelObj != null);
                ...
                {
                    // Log can't operate on a null channel and exit method
                    this._writeCommandLog("CAN NOT RUN FIVE BAUD INIT ON NULL CHANNELS!", LogType.ErrorLog);
                    ResponseBytes = null; return false;
                }
            }
            ...
            this._writeCommandLog($"USING DEVICE INSTANCE {this.DeviceName} FOR BUFFER OPERATIONS", LogType.InfoLog);
            this._writeCommandLog($"SENDING CONFIGURATION BYTE {InputByte.ToString("X")} FOR CHANNEL ID: {ChannelInUse.ChannelId}!", LogType.WarnLog); 
        ...
        public bool PTFastInit(byte[] InputBytes, bool RequiresResponse, out byte[] ResponseBytes, int ChannelId = -1)
        {
            ...
            // Log Clearing RX buffer, clear it and return 
            J2534Channel ChannelInUse = this._defaultChannel;
            this._writeCommandLog($"ISSUING FAST INIT FOR DEVICE {this.DeviceName} NOW...", LogType.InfoLog);
            if (ChannelId != -1)
            {
                // Check our Device Channel ID
                ChannelInUse = DeviceChannels?.FirstOrDefault(ChannelObj => ChannelObj != null);
                ...
                {
                    // Log can't operate on a null channel and exit method
                    this._writeCommandLog("CAN NOT FAST INIT ON NULL CHANNELS!", LogType.ErrorLog);
                    ResponseBytes = null; return false;
                }
            }
            ...
            this._writeCommandLog($"USING DEVICE INSTANCE {this.DeviceName} FOR BUFFER OPERATIONS", LogType.InfoLog);
            this._writeCommandLog($"SENDING CONFIGURATION BYTES {InputByteString} FOR CHANNEL ID: {ChannelInUse.ChannelId}!", LogType.WarnLog);
        ...
        public bool PTWriteMessages(PassThruStructs.PassThruMsg MessageToSend, uint SendTimeout = 100, int ChannelId = -1)
        {
            ...
            if (this.DeviceChannels.All(ChannelObj => ChannelObj == null)) {
                this._writeCommandLog("CAN NOT ISSUE WRITE COMMANDS ON A DEVICE WITH NO OPENED CHANNELS!", LogType.ErrorLog);
                return false;
            }
            ...
            if (ChannelId != -1)
            {
                ...
                {
                    // Log can't operate on a null channel and exit method
                    this._writeCommandLog("CAN NOT CLEAR WRITE MESSAGES TO NULL CHANNELS!", LogType.ErrorLog);
                    return false;
                }
            }

            // Log information out and prepare to wrtie
            this._writeCommandLog($"SENDING MESSAGES ON CHANNEL WITH ID: {ChannelInUse.ChannelId}", LogType.InfoLog);
            this._writeCommandLog($"ISSUING PASSTHRU WRITE COMMAND WITH TIMEOUT AND MESSAGE: {SendTimeout}ms - 1 MESSAGES", LogType.InfoLog);

            // Issue command, log output and return.
            uint MessagesSent = ChannelInUse.PTWriteMessages(MessageToSend, SendTimeout);
            this._writeCommandLog($"SENT A TOTAL OF {MessagesSent} OUT OF AN EXPECTED 1 MESSAGE!", LogType.WarnLog);
            if (MessagesSent != 1) { this._writeCommandLog("ERROR! FAILED TO SEND OUT THE REQUESTED PT MESSAGE!", LogType.ErrorLog); }
        ...
        public bool PTWriteMessages(PassThruStructs.PassThruMsg[] MessageToSend, uint SendTimeout = 100, int ChannelId = -1)
        {
            ...
            // Log information. If all channels are null, then exit.
            J2534Channel ChannelInUse = this._defaultChannel;
            this._writeCommandLog($"ISSUING PASSTHRU WRITE COMMAND FOR {MessageToSend.Length} MESSAGES WITH TIMEOUT", LogType.InfoLog);
            if (ChannelId != -1)
            {
                // Check our Device Channel ID
                ChannelInUse = DeviceChannels?.FirstOrDefault(ChannelObj => ChannelObj != null);
            ...
            this._writeCommandLog($"SENDING MESSAGES ON CHANNEL WITH ID: {ChannelInUse.ChannelId}", LogType.InfoLog);
            foreach (var MsgObject in MessageToSend) { this._writeCommandLog($"\tISSUING MESSAGE: {BitConverter.ToString(MsgObject.Data)}", LogType.TraceLog); }

            // Issue command, log output and return.
            uint MessagesSent = ChannelInUse.PTWriteMessages(MessageToSend, SendTimeout);
            this._writeCommandLog($"SENT A TOTAL OF {MessagesSent} OUT OF AN EXPECTED {MessageToSend.Length} MESSAGES!", LogType.WarnLog);
            if (MessagesSent != MessageToSend.Length) { this._writeCommandLog("ERROR! FAILED TO SEND OUT THE REQUESTED PT MESSAGE!", LogType.ErrorLog); }
        ...
        public PassThruStructs.PassThruMsg[] PTReadMessages(uint MessagesToRead = 1, uint ReadTimeout = 250, int ChannelId = -1)
        {
            ...
            J2534Channel ChannelInUse = this._defaultChannel;
            this._writeCommandLog($"ISSUING A PTREAD MESSAGES COMMAND FOR A TOTAL OF {MessagesToRead} MESSAGES WITH A TIMEOUT OF {ReadTimeout}", LogType.InfoLog);
            if (ChannelId != -1)
            {
                // Check our Device Channel ID
                ChannelInUse = DeviceChannels?.FirstOrDefault(ChannelObj => ChannelObj != null);
            ...
            this._writeCommandLog($"READING MESSAGES ON CHANNEL WITH ID: {ChannelInstance.ChannelId}", LogType.InfoLog);
            ...
            if (ReadMessages.Length == 0) {
                this._writeCommandLog("ERROR! NO MESSAGES PROCESSED FROM OUR PTREAD COMMAND!", LogType.ErrorLog);
                return Array.Empty<PassThruStructs.PassThruMsg>();
            }

            // Print our messages out and return them.
            this._writeCommandLog("RETURNING OUT CONTENTS FOR MESSAGES PULLED IN NOW!", LogType.InfoLog);
            this._writeCommandLog($"READ A TOTAL OF {ReadMessages.Length} OUT OF {MessagesToRead} EXPECTED MESSAGES", LogType.InfoLog);
            try
            {
                // Build logging output table for field information
                var BuiltStrings = J2534Device.PTMessageToTableString(ReadMessages);
                this._writeCommandLog(BuiltStrings);
            } 
            catch
            {
                // Log them out normally here
                this._writeCommandLog("MESSAGES READ WITHOUT ISSUES! PRINTING THEM OUT BELOW IN HEX FORMAT NOW...", LogType.InfoLog);
                this._writeCommandLog(string.Join("\t-->", ReadMessages.Select(MsgObj => BitConverter.ToString(MsgObj.Data))));
            }
            if (MessagesToRead != ReadMessages.Length) this._writeCommandLog("WARNING! READ MISMATCH ON MESSAGE COUNT!", LogType.WarnLog);
            return ReadMessages;
        }
        ...
        public bool PTSelect(out PassThruStructs.SChannelSet SelectedChannelSet, int ChannelId = -1)
        {
            ...
            this._writeCommandLog($"ISSUED A PTSELECT FOR PHYSICAL CHANNEL {ParentPhysicalChannel.ChannelId} WITHOUT ISSUES!", LogType.InfoLog);
            return true;
        }
        /// <summary>
        ...
        public bool PTQueueMessages(uint LogicalChannelId, PassThruStructs.PassThruMsg MessageToWrite, int PhysicalChannelId = -1)
        {
            ...
            if (LogicalChildChannel == null) {
                this._writeCommandLog($"NO LOGICAL COMMAND WITH ID {LogicalChannelId} COULD BE FOUND!", LogType.ErrorLog);
                return false;
            }
            ...
            this._writeCommandLog($"ISSUED A PTQUEUE MESSAGES COMMAND TO PHYSICAL CHANNEL {ParentPhysicalChannel.ChannelId} AT LOGICAL CHANNEL {LogicalChildChannel.ChannelId} OK!", LogType.InfoLog);
            return true;
        }
        /// <summary>
        /// Issues a PTQueue messages command on a desired logical channel.
        ...
        public J2534Filter PTStartMessageFilter(ProtocolId FilterProtocol, FilterDef FilterType, string Mask, string Pattern, string FlowControl = null, TxFlags FilterFlags = 0x00, int ChannelId = -1)
        {
            ...
            if (this.DeviceChannels.All(ChannelObj => ChannelObj == null)) {
                this._writeCommandLog("CAN NOT ISSUE FILTER COMMANDS ON A DEVICE WITH NO OPENED CHANNELS!", LogType.ErrorLog);
                return null;
            }
            ...
            // Find the channel to use and send out the command.
            this._writeCommandLog($"ISSUING A PASSTHRU FILTER ({FilterType}) COMMAND NOW", LogType.InfoLog);
            this._writeCommandLog($"STARTING FILTER ON CHANNEL WITH ID: {ChannelInUse.ChannelId}", LogType.InfoLog);

            // Issue command, log output and return.
            J2534Filter OutputFilter = ChannelInUse.StartMessageFilter(FilterProtocol, FilterType, Mask, Pattern, FlowControl, FilterFlags);
            if (OutputFilter != null) this._writeCommandLog($"STARTED NEW FILTER CORRECTLY! FILTER ID: {OutputFilter.FilterId}", LogType.InfoLog);
            this._writeCommandLog("FILTER OBJECT HAS BEEN STORED! RETURNING OUTPUT CONTENTS NOW");
            return OutputFilter;
        }
        /// <summary>
        /// Builds a new Message filter from a set of input data and returns it. Passed out the Id of the filter built.
        ...
        public J2534Filter PTStartMessageFilter(J2534Filter FilterToStart, int ChannelId = -1)
        {
            ...
            // Find the channel to use and send out the command.
            this._writeCommandLog($"ISSUING A PASSTHRU FILTER ({FilterToStart.FilterType}) COMMAND NOW", LogType.InfoLog);
            this._writeCommandLog($"STARTING FILTER ON CHANNEL WITH ID: {ChannelInUse.ChannelId}", LogType.InfoLog);
        ...
        public bool PTStopMessageFilter(uint FilterId)
        {
            ...
            if (LocatedFilter == null) {
                this._writeCommandLog($"ERROR! NO FILTERS FOUND FOR THE GIVEN FILTER ID OF {FilterId}", LogType.ErrorLog);
                return false;
            }
            ...
            for (int ChannelIndex = 0; ChannelIndex < this.DeviceChannels.Length; ChannelIndex++)
            {
                ...
                this._writeCommandLog($"STOPPING FILTER ID {FilterId} ON CHANNEL {ChannelIndex} (ID: {this.DeviceChannels[ChannelIndex].ChannelId}) NOW!", LogType.InfoLog);
                this.DeviceChannels[ChannelIndex].StopMessageFilter(LocatedFilter);
                return true;
            }

            // If we get here, something is wrong.
            this._writeCommandLog("ERROR! COULD NOT FIND A CHANNEL WITH THE GIVEN FILTER TO STOP ON! THIS IS WEIRD!", LogType.ErrorLog);
            return false;
        }
        /// <summary>
        ...
        public bool PTStopMessageFilter(J2534Filter FilterInstance)
        {
            ...
            if (ChannelFilterSet == null) {
                this._writeCommandLog("ERROR! COULD NOT FIND FILTER OBJECT TO REMOVE FROM OUR INSTANCE!", LogType.ErrorLog);
                return false;
            }

            // Issue the command here.
            int ChannelIndex = this.ChannelFilters.ToList().IndexOf(ChannelFilterSet);
            this._writeCommandLog($"STOPPING FILTER ID {FilterInstance.FilterId} ON CHANNEL {ChannelIndex} (ID: {this.DeviceChannels[ChannelIndex].ChannelId}) NOW!", LogType.InfoLog);
        ...
        public J2534PeriodicMessage PTStartPeriodicMessage(PassThruStructs.PassThruMsg MessageToWrite, uint SendInterval, int ChannelId = -1)
        {
            // Log information. If all channels are null, then exit.
            if (this.DeviceChannels.All(ChannelObj => ChannelObj == null)) {
                this._writeCommandLog("CAN NOT ISSUE PERIODIC COMMANDS ON A DEVICE WITH NO OPENED CHANNELS!", LogType.ErrorLog);
                return null;
            }
            ...
            this._writeCommandLog($"SENDING PERIODIC MESSAGES ON CHANNEL WITH ID: {ChannelInUse.ChannelId}", LogType.InfoLog);
            this._writeCommandLog($"ISSUING PASSTHRU START PERIODIC COMMAND WITH TIMEOUT AND MESSAGE: {SendInterval}ms - 1 MESSAGES", LogType.InfoLog);

            // Issue command, log output and return.
            J2534PeriodicMessage MessageBuilt = ChannelInUse.StartPeriodicMessage(MessageToWrite, SendInterval);
            this._writeCommandLog($"ISSUED A PTSTARTPERIODIC MESSAGE COMMAND TO OUR API INSTANCE", LogType.WarnLog);
            if (MessageBuilt == null) { this._writeCommandLog("ERROR! FAILED TO SEND OUT THE REQUESTED PT MESSAGE!", LogType.ErrorLog); }
            return MessageBuilt;
        }
        /// <summary>
        ...
        public bool PTStopPeriodicMessage(uint MessageId)
        {
            ...
            if (LocatedPeriodicMsg == null) {
                this._writeCommandLog($"ERROR! NO MESSAGES FOUND FOR THE GIVEN MESSAGE ID OF {MessageId}", LogType.ErrorLog);
                return false;
            }
            ...
            for (int ChannelIndex = 0; ChannelIndex < this.DeviceChannels.Length; ChannelIndex++)
            {
                ...
                this._writeCommandLog($"STOPPING PERIODIC MESSAGE WITH ID {MessageId} ON CHANNEL {ChannelIndex} (ID: {this.DeviceChannels[ChannelIndex].ChannelId}) NOW!", LogType.InfoLog);
                this.DeviceChannels[ChannelIndex].StopPeriodicMessage(LocatedPeriodicMsg);
                return true;
            }

            // If we get here, something is wrong.
            this._writeCommandLog("ERROR! COULD NOT FIND A CHANNEL WITH THE GIVEN PERIODIC MESSAGE TO STOP ON! THIS IS WEIRD!", LogType.ErrorLog);
            return false;
        }
        /// <summary>
        /// Stops a Periodic Message based on the ID of the message
        ...
        public bool PTStopPeriodicMessage(J2534PeriodicMessage MessageInstance)
        {
            ...
            for (int ChannelIndex = 0; ChannelIndex < this.DeviceChannels.Length; ChannelIndex++)
            {
                ...
                this._writeCommandLog($"STOPPING PERIODIC MESSAGE WITH ON CHANNEL {ChannelIndex} (ID: {this.DeviceChannels[ChannelIndex].ChannelId}) NOW!", LogType.InfoLog);
        ...
        private string _buildSplittingLine(string SplitChar = "=", int LineSize = 150)
        {
        ...
        private void _writeCommandLog(string Message, LogType Level = LogType.DebugLog, [CallerMemberName] string MemberName = "PT COMMAND")
        {
            // Find the command type being issued. If none found, then just write normal output.
            if (!MemberName.StartsWith("PT")) {
                this._sessionLogger?.WriteLog($"[{MemberName}] ::: {Message}", LogType.InfoLog);
                return;
            }

            // Now write our output contents and exit out
            string FinalMessage = $"[{this.DeviceName}][{MemberName}] ::: {Message}";
            this._sessionLogger?.WriteLog(FinalMessage, Level);
        }
    }
}
